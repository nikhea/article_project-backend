...
exports.get_one_comments = async (req, res, next) => {
	try {
		...
		const article = await db.Article.findById(id).populate('comments').select('comments').exec();
		if (article) {
...
exports.create_new_comment = async (req, res, next) => {
	...
	try {
		...
		if (articles) {
			...
			let article = {};
			(article.content = req.body.content), (article.author = req.body.author);
...
exports.remove_one_comment = async (req, res, next) => {
	try {
		...
		const comment = await db.Comment.remove({ _id: id });
		if (comment) {
